...
library(survival)
library(survminer)
scoreFile="GSE63885_TLSICDscore.txt"     #TLSICDscore打分文件
cliFile="GSE63885_SurvTime.txt"           #生存数据文件
...
#获取最优cutoff
res.cut=surv_cutpoint(data, time="futime", event="fustat", variables=c("TLSICDscore"))
cutoff=as.numeric(res.cut$cutpoint[1])
print(cutoff)
Type=ifelse(data[,"TLSICDscore"]<=cutoff, "Low", "High")
data$group=Type
outTab=rbind(id=colnames(data), data)
write.table(outTab, file="GSE63885_CutoffGroup.txt", sep="\t", quote=F, col.names=F)
...
#保存图片
pdf(file="GSE63885_survival.pdf", onefile = FALSE, width=7, height=5.5)
print(surPlot)
dev.off()
