...
setwd("D:/LUAD/13.ImmuneCor/HHLL")

library(psych)
library(data.table)

CIBER <- fread(paste("D:/LUAD/13.ImmuneCor/HHLL/TCGA_LUAD_Cibersort_NoTcellsCD4.csv",sep=''),header = T)
#CIBER <- t(CIBER)
...
sample_name <- "TCGA"
data<- fread(paste('D:/LUAD/13.ImmuneCor/HHLL/Exp_',sample_name,'_TLS_ICD.csv',sep=''), header = T)
...
rf_risk <- read.csv('D:/LUAD/13.ImmuneCor/HHLL/scale_TCGA_HHLL.csv')
...
pdf(paste("cor_genedown_Low_cell.pdf",sep=''),width =7,height = 22)
pheatmap(data.r, 
         color=myColor,
         breaks=myBreaks,
         clustering_method="average", cluster_rows=F, cluster_cols=F,display_numbers=sig.mat
...
pdf(paste("cor_genedown_High_cell.pdf",sep=''),width =7,height = 22)
pheatmap(data.r, 
         color=myColor,
         breaks=myBreaks,
         clustering_method="average", cluster_rows=F, cluster_cols=F,display_numbers=sig.mat
